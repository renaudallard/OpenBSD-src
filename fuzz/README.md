...
Every run first rebases this branch onto `openbsd/src` master and force
pushes it, so the fuzzing always targets current sources. That only works
while the branch carries nothing but the fuzzing infrastructure: keep
source patches on a branch of their own, otherwise the rebase eventually
hits a conflict and the fork stops tracking upstream.

A conflict fails the sync job, and with it the run, rather than passing
quietly. The fuzzing jobs still run, on whatever was last synced.
...
One issue is opened per distinct title, that is per target and signal.
`scripts/issue.sh` reads the titles of the existing `fuzz-finding` issues
straight from the tracker, closed ones included, so closing an issue does
not bring it back on the next run. A different signal on the same target
still counts as a new finding.
