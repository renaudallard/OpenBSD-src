...
# The issue tracker is the record of what has already been reported, so
# ask it rather than keeping a second copy alongside it.  Pull the titles
# once; an exact match means this finding already has an issue, open or
# closed.
ISSUES=$(mktemp)
trap 'rm -f "$ISSUES"' EXIT
gh issue list --state all --label fuzz-finding --limit 2000 \
    --json title --jq '.[].title' > "$ISSUES"
...
This issue was automatically created by the fuzzing CI."
	...
	if grep -Fxq "$title" "$ISSUES"; then
		echo "${name}: ${crash_n} crash(es), ${ubsan_n} UBSan trap(s) (already reported)"
		continue
	fi
